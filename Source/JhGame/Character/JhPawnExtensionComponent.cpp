...
const FName UJhPawnExtensionComponent::NAME_ActorFeatureName("PawnExtension");

UJhPawnExtensionComponent::UJhPawnExtensionComponent(const FObjectInitializer& ObjectInitializer)
...
void UJhPawnExtensionComponent::OnRegister()
{
	Super::OnRegister();
	...
	// 올바른 Actor에 등록되었는지 확인:
	{
		if (!GetPawn<APawn>())
		{
			UE_LOG(LogJh, Error, TEXT("this component has been added to a BP whose base class is not a Pawn!"));
			return;
		}
	}
...
void UJhPawnExtensionComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	//      - JhPawnExtensionComponent의 경우, 모든 Actor의 Feature 상태 변화에 대해 OnActorInitStateChanged()가 호출됨
	ensure(TryToChangeInitState(FJhGameplayTags::Get().InitState_Spawned));
...
void UJhPawnExtensionComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UJhPawnExtensionComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	if (Params.FeatureName != NAME_ActorFeatureName)
	{
		// JhPawnExtensionComponent는 다른 Feature Component들의 상태가 DataAvailable를 관찰하여, Sync를 맞추는 구간이 있었다 (CanChangeInitState)
		// - 이를 가능케하기 위해, OnActorInitStateChanged에서는 DataAvailable에 대해 지속적으로 CheckDefaultInitialization을 호출하여, 상태를 확인한다
		const FJhGameplayTags& InitTags = FJhGameplayTags::Get();
...
bool UJhPawnExtensionComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	check(Manager);
...
void UJhPawnExtensionComponent::CheckDefaultInitialization()
{
